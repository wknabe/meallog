...
for (const keyword of ['鶏むね肉', '玉ねぎ', '白米', 'ごはん', '鮭', '豚こま', 'ブロッコリー']) {
  ...
  for (const hit of hits) {
    console.log(`   ${hit.name} | ${hit.kcal}kcal P${hit.protein_g} F${hit.fat_g} C${hit.carb_g}`);
  }
}
...
const foodIdByCode = new Map(
  db
    .prepare('SELECT id, std_code FROM foods WHERE std_code IS NOT NULL;')
    .all()
    .map((r) => [r.std_code, r.id]),
);
db.exec('BEGIN;');
let dishCount = 0;
...
for (const [label, sql, params] of [
  [
    '和食 × 主菜',
    "SELECT COUNT(*) AS c FROM dishes WHERE category='main' AND cuisine='japanese'",
    [],
  ],
  [
    '中華 × 主菜',
    "SELECT COUNT(*) AS c FROM dishes WHERE category='main' AND cuisine='chinese'",
    [],
  ],
  ...
  [
    '辛いもの（全体）',
    "SELECT COUNT(*) AS c FROM dishes d JOIN dish_tastes t ON t.dish_id=d.id WHERE t.taste='spicy'",
    [],
  ],
...
db.prepare('INSERT INTO dish_ingredients (dish_id, food_id, grams) VALUES (?, ?, ?);').run(
  dishId,
  foodId,
  150,
);
...
const leftIngredients = db
  ...
  .get(dishId).c;
const leftTastes = db
  .prepare('SELECT COUNT(*) AS c FROM dish_tastes WHERE dish_id = ?;')
  .get(dishId).c;
console.log(
  `--- 連鎖削除: 材料 ${leftIngredients}件 / 味タグ ${leftTastes}件（どちらも0なら正常）`,
);

// ── 単一行テーブルの制約確認 ──
try {
  db.prepare(`INSERT INTO settings (id, updated_at) VALUES (2, ?);`).run(now);
